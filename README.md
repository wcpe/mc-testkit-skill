...
- **mc-testkit 工具本身**：本技能场景下通常是同机 sibling 项目 `D:\Projects\mc-testkit`（模板在其 `template/`、契约真源在 `docs/API.md`），或从 `https://maven.wcpe.top/repository/maven-public/` 消费。接入前请读一眼 mc-testkit 当前 `README.md` / `CHANGELOG.md` 确认版本与能力（本技能撰写时为 v0.3.0），不要照搬文档里的版本号。
